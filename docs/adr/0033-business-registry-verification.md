...
2. **아웃바운드 포트**: 도메인에 `BusinessRegistryVerifier` 포트를 둔다. 인프라 어댑터가 **비즈노 API**(국세청 사업자등록정보 진위·상태)를 호출한다 — 외부 연동·HTTP 세부는 어댑터 안에만 존재한다([ADR-0003](./0003-jwt-auth-after-oauth-login.md)의 `OidcTokenVerifier`/[ADR-0031](./0031-apple-sign-in-authorization-code-flow.md)의 `AppleAuthClient`와 같은 포트/어댑터 패턴).
