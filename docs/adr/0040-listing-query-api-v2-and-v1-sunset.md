...
- 목록 · 지도 · 키워드 검색 · 찜 목록 · 최근 본 → **빈 페이지**(`content: []`, `totalElements: 0`).
...
v1 DTO는 개정 전 구조로 되돌려 유지한다. 항목 DTO는 `content`가 항상 비어 있어 런타임에 직렬화되지 않지만, **응답 봉투와 스펙·Swagger 문서의 정본**이므로 v3 모양 그대로 남긴다.
...
- **부정/트레이드오프**
  ...
  - **v1 DTO를 개정 전(v3) 구조로 복원해 유지해야 한다.** 실데이터를 만들지 않는 DTO 한 벌이 코드에 남고, 그 모양이 스펙·Swagger에 계속 노출된다.
...
- **v1 스텁이 저장소를 호출하지 않는다는 것을 테스트로 강제한다** — 매물 저장소 목에 대해 상호작용 0건을 단정한다. 이 가드가 없으면 "빈 결과인데 왜 DB를 읽지"가 나중에 값 조립으로 되돌아간다.
