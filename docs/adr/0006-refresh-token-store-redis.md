...
1. **저장 형태**: 키 `refresh:{tokenHash}` → 값 `{ userId, status, issuedAt, expiresAt }`(`status` ∈ `ACTIVE`/`ROTATED`/`REVOKED` — 도메인 `RefreshTokenStatus`), **TTL = refresh 만료 시각**. refresh의 구체 만료값(14일 등)은 [ADR-0011](./0011-token-lifetime-and-secret-policy.md)에서 확정한다. 만료 시 폐기(`ROTATED`/`REVOKED`) 레코드까지 자동 소멸하므로 **별도 정리 배치가 불필요**하다. (초안의 boolean `revoked`를 3-상태 `status`로 일반화.)
