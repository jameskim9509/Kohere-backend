...
- 컬렉션 통째 drop 같은 **파괴적 일괄 변경은 금지**다([ADR-0005](./0005-polyglot-persistence.md) D7). 유일한 예외는 매물 스키마 v4 이행 1회다(§6·[ADR-0039](./0039-listing-schema-v4-registration-form.md)).
...
**MongoDB의 시드·카탈로그·문서에 대한 1회성 변경은 "마커 기반 마이그레이션 러너"로 환경당 정확히 1회 자동 적용하며, 그 러너는 자체 구현이 아니라 모듈별 [Mongock] `@ChangeUnit`으로 구현한다(수동 drop+reseed를 대체 — 매물 스키마 v4 이행 1회만 예외, §6).** Mongock은 MySQL의 Flyway에 대응하는 MongoDB 마이그레이션 도구다. 세부는 다음과 같다.
...
6. **비파괴·전진.** **사용자·운영 데이터(`diagnoses` 등)** 는 컬렉션 통째 drop이 아니라 **영향 문서만 교체/이행**한다([ADR-0005](./0005-polyglot-persistence.md) D7). 단 **서버 소유 레퍼런스 카탈로그**(`diagnosisQuestions`·`diagnosisSuggestions`)의 init 베이스라인(order 0000)은 결정성을 위해 **비우고 캐노니컬 시드를 재적재**할 수 있다(레퍼런스 데이터 한정 — 사용자 데이터엔 불가). 마이그레이션은 **신 스키마 코드(엔티티·enum) 이후**에 적용한다 — 일이 "구 데이터를 신 스키마로 이행"이기 때문이다. **1회 예외 — 매물 스키마 v4 이행**: `listings`의 v3 문서를 폐기하고 시드를 **수동 주입**한다([ADR-0039](./0039-listing-schema-v4-registration-form.md)). v4가 새로 요구하는 필드(담당자 연락처·설문 3종 등)에 **대응하는 원본 값이 v3 문서에 없어** 백필·점진 이행으로 표현할 수 없기 때문이다. 예외 조건은 셋이다 — (a) 대상은 `listings`·`listingCatalog` **1회**뿐이고, (b) 삭제·재적재를 애플리케이션 코드가 수행하지 않으며(`0115`는 validator·인덱스만 담당), (c) 근거 ADR과 예외 범위를 각 문서에 명시한다. **재발 방지**: v4 이후의 매물 스키마 변경은 다시 이 항목의 전진·비파괴 규약을 따르며, 예외는 이 1회로 소멸한다.
...
| **F. 수동 drop + reseed (mongosh)** | 도구 불요·단순 | 번거롭고 실수 위험, 환경마다 수작업, 큐레이션·실데이터 보존 불가 | **바로 이 불편이 본 ADR의 동기**. 단 매물 스키마 v4 이행은 백필할 원본 값이 없어 changeUnit으로 표현되지 않으므로 **1회에 한해 이 방식**을 쓴다(§6·[ADR-0039](./0039-listing-schema-v4-registration-form.md)) |

## Consequences

- **긍정**
  - 시드·카탈로그 변경이 **배포로 자동 1회 적용**되어 수동 drop+reseed가 사라진다(본 ADR의 동기 해소). 단 매물 스키마 v4 이행은 예외로, listing 체인을 baseline으로 리셋하고(`0100` 존치, 나머지 삭제) `listings`·`listingCatalog` 시드는 운영자가 수동 주입한다(§4·§6·[ADR-0039](./0039-listing-schema-v4-registration-form.md)).
