...
**정답인 경우** — `correct=true`이며 해설(`explanation`, 사용자 언어 번역)을 함께 반환한다. 정답 키(`correctChoice`)는 노출하지 않는다.
...
**오답인 경우** — `correct=false`이며 정답 키(`correctChoice`)와 해설(`explanation`, 사용자 언어 번역)을 함께 반환한다.
...
> `explanation`은 퀴즈 도큐먼트의 인라인 언어-키 맵에서 사용자 언어 키를 골라(없으면 `en` 폴백) 채운 해설이며, 정답·오답 채점 응답 모두에 포함된다. `correctChoice`는 언어와 무관한 보기 키다.
