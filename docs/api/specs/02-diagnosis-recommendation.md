...
② 입국 목적은 유학 여부(`STUDY`/`NON_STUDY`) 분기이며, 이에 따라 ③ 단계가 대학 선택(유학) 또는 지역(구) 선택(비유학)으로 갈린다. 진단 문항·선택지는 앱이 하드코딩하지 않고 백엔드가 제공한다. 클라이언트는 `POST /api/v2/diagnoses/start`로 진단을 시작하고 `POST /api/v2/diagnoses/next`로 답을 하나씩 보내며, 서버가 진행 위치를 보고 다음 질문을 정한다(클라이언트는 `step`을 지정하지 않는다). ③ 단계의 대학/지역 분기도 **서버가 저장된 `purpose`로 결정**해 한쪽 질문만 내려준다. 요청에 누적 답을 묶어 다시 보내지 않는다 — 진행 상태는 서버가 DB에 들고 있다. 표시 라벨은 사용자 표시 언어로 번역되어 내려간다(US-2-5·US-2-6).

> **v2 서버 주도 흐름(issue #157)**: 클라이언트가 **`POST /api/v2/diagnoses/start`** 로 시작한 뒤 **`POST /api/v2/diagnoses/next`** 를 반복 호출하면 서버가 진행 위치로 다음 질문을 결정하는 **서버 주도 대화형 흐름**을 `/api/v2`에 신설한다 — **서버는 질문·분기만 주도**하고 진단을 시작하는 시점도, 확정된 매물을 조회하는 시점도 클라이언트가 정한다(확정 응답은 `diagnosisId`만 주고 매물은 클라이언트가 `GET /api/v2/diagnoses/{id}/recommendations`로 별도 조회). 서버가 미리 필터링하는 지점은 **① 지역 하나뿐**이고(0건이면 카탈로그의 "다른 지역?" 문항을 일반 질문으로 끼워 넣음), 빌더 완성 시 자동 확정하되 **매칭은 조회하지 않는다** — 매칭 0건인지는 클라이언트가 추천을 조회한 응답의 `resultCode: NO_MATCH`로 알려준다(조정 제안 문구·액션은 없음). 상세는 아래 **[v2 — 서버 주도 진단 흐름](#v2--서버-주도-진단-흐름-issue-157)** 절, 결정은 [ADR-0036](../../adr/0036-diagnosis-v2-server-driven-flow.md), 시퀀스는 [US-2-7](../../architecture/sequence-diagrams/02-diagnosis-recommendation/us-2-7-v2-server-driven-flow.md).
...
> 진단의 **v2 엔드포인트만 비회원(게스트)에게 연다**. **조회 3종(§4~§6)은 회원 전용으로 유지**하며 게스트 진단의 정본 경로는 v2뿐이다. 이 절이 게스트 인가·신원·언어·세션 키·소유권 규칙의 정본이다. **기존 회원 경로의 계약(소유권·언어·응답 형태·403/404 의미)은 바뀌지 않는다** — 게스트 경로가 추가되는 것이다.
...
| 발급 시점 | **`POST /api/v2/diagnoses/start` 응답 하나뿐**. 조회 3종에는 게스트 키 발급 지점이 없다 — 회원 전용이다 |
...
> **조회 3종(§4~§6)은 이 표에 없다.** 회원 전용이라 게스트가 호출할 수 없고(토큰 없으면 401 `UNAUTHENTICATED`), 따라서 그 엔드포인트는 `X-Guest-Session-Id`를 **읽지도 발급하지도 않는다**. 회원 요청에 이 헤더가 실려 와도 무시된다.
>
...
### 진단 조회 3종은 회원 전용 — 게스트 진단은 v2로 닫힌다

**진단 조회 3종(§4~§6, `GET /api/v1/diagnoses`·`/latest`·`/{diagnosisId}`)은 회원 전용으로 유지한다.** 게스트에게 열지 않으므로 `SecurityConfig`에 이 경로용 `permitAll` 매처를 추가하지 않고, 현행대로 `.anyRequest().authenticated()`에 남는다 — 토큰 없이 호출하면 **401 `UNAUTHENTICATED`** 다.

귀결:

- **게스트 진단은 이 셋에 애초에 걸리지 않는다.** 이력·최근은 사용자 id 기준 질의라 `userId`가 없는 게스트 문서가 후보에 들어오지 않고, 상세는 소유권 검사가 막는다(신원 종류가 다르면 거절).
- **게스트용 이력·최근 시맨틱을 정의할 필요가 없다** — 게스트는 이 엔드포인트에 도달하지 못한다(401). 한 게스트 세션 키에 달리는 종료 진단이 최대 1건이라(`/start`가 매번 새 키를 발급한다) 이력이라는 개념 자체가 성립하지 않는다.

**게스트 진단 흐름은 v2 엔드포인트로 닫힌다:**
...
이슈 #181의 완료 조건("비로그인 사용자가 챗봇 진단 API 호출 시 2xx")은 이 흐름으로 충족된다.
...
>
> 이 표는 **소유권 검사가 도는 지점**(§6 상세·v2-3 추천)에 적용된다. 다만 **게스트 행(키 `K1`·키 없음)이 실제로 도달할 수 있는 지점은 v2-3 추천 하나뿐**이다 — §6은 회원 전용이라 토큰 없는 요청이 소유권 검사 이전에 401로 걸리고, 회원 행(`userId=A`↔`B`)만 §6에서 실현된다. "회원(`userId=A`)이 게스트 진단을 조회 → 403"은 회원이 v2에서 만들어진 게스트 진단 id를 §6이나 v2-3으로 찔러 볼 때 성립한다.
>
...
| ⑥ ARC 발급 여부 | `arcStatus` | enum (단일) | `ARC_ISSUED`, `NO_ARC` | 필수, 1택. `NO_ARC`(미발급)이면 추천이 매물 루트 `arcRequired=NOT_REQUIRED`를 필터로 적용한다(v2-3). `ARC_ISSUED`이면 ARC 필터를 적용하지 않는다 |
...
> **진단 조회 3종의 인증은 모두 "필수"(회원 전용)** 다 — 토큰이 없거나 위조면 401 `UNAUTHENTICATED`, 만료면 401 `TOKEN_EXPIRED`다. 비회원(게스트)이 호출할 수 있는 것은 [v2 엔드포인트](#v2--서버-주도-진단-흐름-issue-157)뿐이며, 근거·범위는 [게스트 접근](#게스트-접근--비회원-진단-issue-181) 절이 정본이다.
>
> 이 셋이 읽는 대상은 **v2 서버 주도 흐름이 확정해 저장한 진단**이다 — 진단을 시작하고 문항에 답하고 확정하는 경로는 v2에만 있다.
...
진단 단건의 입력 전체를 반환한다(지난 진단 다시 보기). **본인 소유의 확정된 진단만** 조회 가능하다.
...
- **확정(`COMPLETED`)된 진단만 조회된다** — 폐기 기록(`DISCARDED`)과 미완주 초안은 `404 DIAGNOSIS_NOT_FOUND`다. 진단 id가 순차 채번이라 소유권만으로는 막히지 않으므로 상태로 한 번 더 거른다.
- **상태 검사(404)가 소유권 검사(403)보다 먼저 돈다.** 순서를 뒤집으면 타인의 폐기·미확정 진단이 403이 되어 그 id에 진단이 있다는 신호가 새로 생긴다.
...
> 진단을 시작하고 문항에 답하고 확정하는 **정본 경로**다. 위 조회 3종(§4~§6)이 읽는 진단을 만드는 곳이기도 하다. 결정: [ADR-0036](../../adr/0036-diagnosis-v2-server-driven-flow.md) · 시퀀스: [US-2-7](../../architecture/sequence-diagrams/02-diagnosis-recommendation/us-2-7-v2-server-driven-flow.md) · 유저 스토리: [US-2-7](../../requirements/user-stories.md).
...
> **게스트**: v2 엔드포인트는 모두 토큰 없이 호출할 수 있다. `/start`가 게스트 세션 키(`X-Guest-Session-Id`, `anonymous<uuid>`)를 응답으로 발급하고, `/next`와 추천 조회는 그 키를 헤더로 에코받는다 — 상세는 [게스트 접근](#게스트-접근--비회원-진단-issue-181) 절.
...
| --- | --- | --- |
| `NEXT_QUESTION` | 다음 질문이 남음(마지막 슬롯 전). ① 지역 0건 예외질문(`field: "regionRetry"`)도 **이 코드**로 내려간다 | `question`(그 단계 문항 1개) |
...
> 게스트 개방으로 신규 도메인 에러 코드를 만들지 않는다 — 세션 키 문제는 기존 `DIAGNOSIS_SESSION_NOT_FOUND`(400), 소유권 위반은 공통 `FORBIDDEN`(403)에 흡수된다([게스트 접근](#게스트-접근--비회원-진단-issue-181)). **`UNAUTHENTICATED`(401)가 발생하지 않는 것은 게스트에게 연 v2 엔드포인트뿐**이며 거기서는 `TOKEN_EXPIRED`(401)만 남는다 — 만료 토큰은 게스트로 강등하지 않기 때문이다. **회원 전용으로 유지되는 조회 3종(§4~§6)은 토큰이 없거나 위조면 그대로 `UNAUTHENTICATED`(401)** 다.
>
