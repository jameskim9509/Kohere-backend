...
임대인은 앱과 별개로 **매물을 등록하는 웹 클라이언트**로도 들어온다. 웹은 소셜 로그인을 지원하지 않고 **이메일(로그인 ID) + 비밀번호**로 가입·로그인한다 — 앱 자격증명(`social_accounts`)과 웹 자격증명(`local_accounts`)이 **하나의 `users` 행에 병렬로 매달리는** 구조다([ADR-0047](../../adr/0047-web-local-credentials-and-phone-based-account-linking.md)). 웹 트랙의 엔드포인트는 §1-1(가입용 SMS 발송) · §1-2(가입용 SMS 확인) · §1-11(가입용 이메일 인증번호 발송) · §1-12(가입용 이메일 인증번호 확인) · §1-3(회원가입) · §1-4(로그인) · §1-5(이메일 찾기용 SMS 발송) · §1-6(이메일 찾기용 SMS 확인) · §1-7(이메일 찾기) · §1-8(재설정 링크 발송) · §1-9(재설정 토큰 확인) · §1-10(재설정 확정) 열둘이고, **모두 비로그인 진입점이라 인증이 불필요(permitAll)** 하다. 앞의 여섯은 계정을 만들어 들어가는 경로이고, **뒤의 여섯은 들어가지 못하게 된 계정을 되찾는 경로**다 — §1-5~§1-7이 잊어버린 로그인 ID(이메일)를 찾아 주고, §1-8~§1-10이 비밀번호 재설정과 계정 잠금 해제를 **하나의 흐름으로** 처리한다(해제 전용 API를 따로 두지 않는 이유는 §1-8).

> **번호 순서와 흐름 순서가 다르다.** 가입 흐름은 **§1-1·§1-2(연락처 인증) → §1-11·§1-12(이메일 인증) → §1-3(가입 제출)** 이며, 이메일 인증이 §1-3 뒤 번호를 받은 것은 **§1-3~§1-10이 이미 코드·테스트·시퀀스·`.http`에 인용돼 있어 재배열이 그 참조를 통째로 깨기 때문**이다(번호는 추가만 하고 밀지 않는다). **두 인증은 서로 순서가 무관**하고 — 각각 다른 키스페이스의 마커를 남길 뿐이라 어느 쪽을 먼저 해도 된다 — 가입 제출 전에 **둘 다** 살아 있으면 된다.

관련 유저 스토리: US-1-11(웹 회원가입)·US-1-12(웹 로그인)·US-1-13(가입용 연락처 인증)·US-1-15(앱 온보딩 시 웹 계정 병합)·US-1-16(이메일 찾기)·US-1-17(비밀번호 재설정으로 로그인 복구)·US-1-18(가입용 이메일 인증).
...
- **계정 열거 가능(둘째 경로)** — 가입용 이메일 인증 발송(§1-11)이 이미 가입된 주소에 `409`를 주므로, **선행 게이트 없이 임의의 주소에 대해** 가입 여부를 물을 수 있다. 가입 폼에서 즉시 "이미 사용 중"을 알리고 남의 메일함으로 가는 인증번호를 막기 위해 수용했으며, **그 결과 §1-8이 세워 둔 열거 방어는 이 경로로 우회된다**(§1-8·§1-11).
...
| POST | `/api/v1/auth/email/signup/verification-code` | 가입용 이메일 인증번호 발송(임대인 웹·비로그인) — 이메일 키 챌린지. **이미 가입된 로그인 ID면 발송하지 않고 409** | 불필요 | 200 |
| POST | `/api/v1/auth/email/signup/verify` | 가입용 이메일 인증번호 확인(임대인 웹·비로그인) → 이메일 키 검증 마커 저장 | 불필요 | 200 |
| POST | `/api/v1/auth/signup` | 임대인 웹 회원가입(연락처·이메일 인증 선행 · 한 트랜잭션으로 ACTIVE 완주 + 같은 번호의 기존 앱 계정과 연동), refresh는 쿠키 | 불필요 | 200 |
...
> **웹 트랙 12개 경로(§1-1~§1-12)는 인증 불필요(permitAll)** 다 — 계정이 없거나, 로그인 이전 단계이거나, **로그인할 수 없게 된 상태에서 부르는 복구 경로**라서다. 보안 설정에서는 `SecurityConfig`의 permitAll 매처와 **`PublicPaths.ALL` 두 곳 모두에 등록**해야 하며, **경로를 새로 여는 커밋마다 두 곳을 함께 본다.** 한쪽만 넣으면 **토큰 없이 부르는 로컬·통합 테스트는 전부 초록이고**, 만료된 access 토큰이 남아 있는 브라우저에서만 `401 TOKEN_EXPIRED`가 난다 — 실패가 특정 클라이언트 상태에서만 재현돼 배포한 뒤에야 드러난다(#181이 고친 것과 같은 버그).
...
**선행 조건은 인증 둘이다 — §1-1·§1-2의 SMS 인증과 §1-11·§1-12의 이메일 인증.** 제출된 번호의 검증 마커가 없으면 `422 AUTH_PHONE_NOT_VERIFIED`로 거절하고 **계정 생성도 연동도 하지 않는다** — 번호는 비밀이 아니므로, 인증 없이 번호만으로 기존 계정에 자격증명을 붙일 수 있으면 그 계정의 매물·예약·신청자 PII까지 통째로 탈취된다. 번호 소유 증명은 전적으로 SMS 인증이 담당한다.

제출된 이메일의 검증 마커가 없으면 `422 AUTH_EMAIL_NOT_VERIFIED`다. **이쪽이 지키는 것은 남의 계정이 아니라 본인의 복구 경로**다 — 이메일은 웹 로그인 ID이자 비밀번호 재설정 메일이 도착할 유일한 주소이고, 재설정이 계정 잠금 해제를 겸한다(§1-8). 오타로 가입하면 그 계정은 **잠기는 순간 본인이 풀 방법이 사라져** 운영자가 DB를 직접 고치는 것 외에 길이 없다. 두 인증은 **서로 순서가 무관**하며 제출 시점에 둘 다 살아 있으면 된다.
...
> **검증 게이트 우선순위**: ① 제출 `phoneNumber`의 가입용 인증 마커 부재 → `422 AUTH_PHONE_NOT_VERIFIED` → ② 제출 `email`의 가입용 인증 마커 부재 → `422 AUTH_EMAIL_NOT_VERIFIED` → ③ 필수 약관 2종 미동의 → `422 AUTH_REQUIRED_AGREEMENT_MISSING` → ④ `local_accounts.email` 중복 → `409 AUTH_EMAIL_ALREADY_REGISTERED` → ⑤ 번호로 매칭된 계정에 이미 `local_accounts` 행이 있음 → `409 AUTH_WEB_ACCOUNT_ALREADY_EXISTS` 순으로 판정한다.
> **①과 ② 사이에는 지켜야 할 순서가 없다** — 둘 다 부작용 없는 Redis 읽기이고 둘 다 `422`이며 어느 쪽도 계정 존재를 드러내지 않는다. 갈리는 것은 "둘 다 미인증"일 때 나가는 코드 하나뿐이라 **기존 계약을 유지하는 쪽으로 SMS를 먼저** 본다. **계약인 것은 「마커 둘이 ④⑤보다 앞」** 이고, 특히 ⑤는 §1-11이 흘리지 않는 정보(**그 번호에 웹 계정이 있는가**)를 드러내므로 마커 게이트가 유일한 방어다.
> **④는 §1-11이 발송 단계에서 먼저 걸러도 그대로 남는다.** 발송~제출 사이 최대 30분(마커 TTL) 동안 남이 같은 주소로 가입을 마칠 수 있고, 유일성을 실제로 보장하는 것은 응답이 아니라 `uq_local_accounts_email`이다. 이 게이트를 빼면 그 창에서 나가는 코드가 `AUTH_EMAIL_ALREADY_REGISTERED`(→ **다른 이메일을 쓰라**)에서 `RESOURCE_CONFLICT`(→ **잠시 후 재시도하라**)로 바뀌는데, 이메일 중복은 재시도로 **절대** 풀리지 않으므로 그건 거짓 안내다. 어느 단계에서 실패하든 **DB 쓰기 전체를 롤백**해 `users` 행만 생기고 자격증명이 없는(= 로그인 불가) 상태나 그 반대를 남기지 않는다. 이 원자성은 **MySQL 쓰기에만** 걸린다 — 커밋 시점에 실패하는 요청(아래 `409 RESOURCE_CONFLICT`)은 토큰이 이미 발급된 뒤라 **쓰이지 않을 refresh 해시 하나가 Redis에 14일 TTL로 남는다**(원문은 응답으로 나가지 않아 악용 불가, 항목은 스스로 만료 — §5-2 알려진 제약과 같은 모양이다).
...
| 422 | `AUTH_EMAIL_NOT_VERIFIED` | 제출 `email`의 가입용 인증 마커가 없거나 만료(§1-11·§1-12 선행) — 계정 생성·연동 모두 하지 않는다 |
...
> — **계정 열거가 가능하다.** `error.details`가 등록된 계정의 비밀번호 불일치에만 실리므로, 임의의 비밀번호로 한 번 호출해 보면 그 이메일의 가입 여부를 알 수 있다. **해제 경로가 생긴 뒤에도 이 필드를 그대로 둔다** — 풀 수 있다는 것과 잠기지 않는 것은 다르고, 잠근 뒤에 메일함을 다녀오게 하는 것보다 **잠기기 전에 "두 번 남았다"를 보여 주어 멈추게 하는 편이 낫다.** 그 대가로 열거는 **다시 한 번 명시적으로 수용**한다. 잠긴 계정은 이미 `423`으로 존재를 드러내고 있고, 시도 레이트리밋은 이메일 축이 이메일 단위라 열거를 막지 못하며 IP 축은 위조 가능하므로 완화책이 아니다. **§1-8은 여전히 같은 선택을 하지 않는다**(가입 여부를 응답으로 가르지 않는다) — 다만 §1-11이 선행 게이트 없는 자리에서 `409`로 가입 여부를 알려 주게 되면서, **이 API 전체에서 "열거를 막고 있다"고 말할 수 있는 엔드포인트는 더 이상 없다.** §1-8이 계속 감추는 것은 깊이 방어이지 열거 차단이 아니다.
...
> **같은 가입 흐름의 §1-11은 반대로 간다**(이미 가입된 이메일이면 `409`). 실수가 아니라 채널마다 감출 값이 달라서다 — **번호**는 뒤에 §1-7의 이름 대조 게이트가 남아 있어 여기서 흘리면 그 게이트가 통째로 무의미해지지만, **이메일**은 웹 로그인 ID라 "이미 쓰이고 있다"를 폼에서 바로 알려 주는 값이 더 크고, 무엇보다 감추면 **남의 메일함으로 인증번호가 발송된다**(SMS와 달리 이메일 채널은 그 발송 자체가 피해다). 열거 수용은 §개요 「알려진 제약」에 적어 두었다.
...
> **그 방어는 이제 반쪽도 못 된다.** 첫째로 §1-11(가입용 이메일 인증번호 발송)이 **선행 게이트 없는 같은 자리에서** 이미 가입된 주소에 `409`를 주므로, 여기서 감춘 사실을 **옆 엔드포인트가 그대로 알려 준다** — 이 절이 막으려던 "완전한 가입 여부 오라클"은 §1-11로 성립한다. 둘째로 메일 발송이 **동기**라 가입된 이메일은 SMTP 왕복 시간만큼 응답이 늦고 발송이 실패하면 `502`가 나가는 반면, 미가입 이메일은 아무 일도 하지 않고 즉시 `200`이다 — **응답 시간과 status 분포로도 존재가 샌다.** 그럼에도 **여기서 응답을 가르지는 않는다**: 우회로가 생겼다는 것과 이 엔드포인트가 직접 오라클이 되는 것은 다르고, §1-11의 열거는 레이트리밋 뒤에 있어 관찰마다 예산을 쓴다(깊이 방어). 레이트리밋(이메일 5회/1시간 · IP 20회/1시간)은 이 누출의 완화책이 **아니다**. 그것은 발송비·메일 폭탄 남용을 막는 장치이고, 한 주소당 한 번만 재 보면 되는 관찰에는 아무 방해가 되지 않는다. 응답을 균일하게 만드는 정공법은 발송을 큐로 밀어내는 것이지만 이번 범위에서 도입하지 않는다 — **본문이 같다는 수준까지만 방어하고 타이밍 누출은 알면서 남긴다.**
...
### 1-11. POST `/api/v1/auth/email/signup/verification-code` — 가입용 이메일 인증번호 발송(임대인 웹·비로그인)

웹 회원가입(§1-3) **전에** 입력한 이메일이 본인 메일함인지 증명하기 위해 인증번호를 발송한다(US-1-18). 인증번호 정책(6자리·코드 TTL 5분·검증 마커 TTL 30분·검증 시도 5회·재발송 간격 60초 — `app.email.*`)과 `VerificationEmailSender` 포트는 정식 사용자용 §3과 **동일하게 재사용**하지만, **챌린지 키가 다르다** — §3은 `email-verify:code:{userId}`라 로그인이 선행돼야 하는데 웹 가입자는 아직 계정이 없다. 그래서 이 경로는 **이메일 키 챌린지**(`signup-email:code:{정규화이메일}`)를 쓰고 **인증 불필요(permitAll)** 경로로 연다. 서버는 입력 이메일을 **소문자로 접고 앞뒤 공백을 지워** 정규화한 뒤 키로 쓴다.

메일은 §3과 같이 **동기 발송**하며 **발송에 성공한 뒤에만** 챌린지를 저장한다 — SMTP 장애·타임아웃이면 챌린지를 만들지 않고 `502 UPSTREAM_ERROR`로 응답해 클라이언트가 재시도하도록 한다.

> **이 엔드포인트는 로그인 ID 중복을 여기서 판정한다.** 이미 `local_accounts.email`에 그 주소가 있으면 **메일을 보내지 않고** `409 AUTH_EMAIL_ALREADY_REGISTERED`로 끊는다. 가입 폼에서 즉시 "이미 사용 중"을 알리는 값이 크고, 무엇보다 **남의 메일함으로 가는 인증번호 메일을 원천 차단**한다.
> **그 대가로 계정 열거를 수용한다.** 선행 게이트가 없는 permitAll 경로가 가입 여부를 응답으로 가르므로, 이 엔드포인트는 임의의 주소에 대해 **가입 여부 오라클**이 된다 — §1-8이 같은 자리에서 `200`으로 감추려는 사실을 여기서 `409`로 알려 주는 셈이라 **그쪽 방어는 이 경로로 우회된다**(§1-8 블록쿼트에 함께 적어 두었다). 명시적으로 받아들인 결과다.
> **판정 순서가 계약이다 — ① 재발송 쿨다운 → ② 이메일·IP 시간당 한도 → ③ 중복 검사 → ④ 발급·발송 → ⑤ 챌린지 저장.** 쿨다운을 한도보다 먼저 보는 것은 60초 안의 재요청(버튼 두 번 누르기)으로 시간당 한도까지 깎지 않기 위해서고(§1-1과 동일), **중복 검사를 한도 뒤에 두는 것은 열거 관찰 한 번이 예산 한 칸을 쓰게 하기 위해서다.** 순서를 뒤집으면 카운터를 하나도 올리지 않고 무한히 물어볼 수 있고, 그 판정은 익명 호출자가 유발하는 DB 읽기다. 다만 한도는 **비용 상한이지 열거 방어가 아니다** — 이메일 축은 한 주소당 한 번만 물으면 되는 관찰에 무력하고, 실제로 묶는 IP 축은 `X-Forwarded-For`라 위조 가능하다.
> **비로그인 permitAll 경로라 발송 남용 방지가 필수다.** **이메일 단위 + IP 단위 이중 레이트리밋**을 건다 — 재발송 간격 60초, **같은 이메일 5회/1시간**, **같은 IP 20회/1시간**. 초과는 모두 `429 TOO_MANY_REQUESTS`이며 어느 축에 걸렸는지 구분해 알리지 않는다(한도 역산 방지). 가입용 SMS(§1-1)·이메일 찾기(§1-5)·재설정 링크(§1-8)와 **예산을 나눠 갖는다** — 공유하면 사용자가 화면을 오가는 것만으로 서로의 몫을 태운다.
> **앱 심사용 고정 인증번호 우회는 이 경로에 적용하지 않는다** — 그 정책은 `userId`와 Google 소셜 계정을 기준으로 판정하는데 웹 가입은 둘 다 없는 단계다(§1-1과 같은 판단).
...
> `expiresIn`은 인증번호 만료까지의 초(seconds). `email`은 마스킹해 반환한다(error-response-guide §6). 인증번호 원문은 응답·로그에 노출하지 않는다.
...
| 400 | `MALFORMED_REQUEST` | JSON 파싱 불가/타입 불일치 |
| 409 | `AUTH_EMAIL_ALREADY_REGISTERED` | 그 주소로 이미 `local_accounts` 행이 있음(웹 로그인 ID 중복). **메일을 보내지 않고 챌린지도 만들지 않는다.** 로그인(§1-4)이나 다른 이메일 입력으로 유도한다 |
| 429 | `TOO_MANY_REQUESTS` | 재발송 간격 60초 미만, 같은 이메일 5회/1시간 초과, 같은 IP 20회/1시간 초과 |
| 502 | `UPSTREAM_ERROR` | 메일 발송 실패(SMTP 장애·타임아웃). 챌린지 미저장, 클라이언트 재시도 유도(공통 코드 — [error-response-guide](../error-response-guide.md) §3) |

---

### 1-12. POST `/api/v1/auth/email/signup/verify` — 가입용 이메일 인증번호 확인(임대인 웹·비로그인)

§1-11로 발송된 인증번호를 검증한다(US-1-18). 성공하면 **이메일 키 검증 마커**(`signup-email:verified:{정규화이메일}`, TTL 1800초)를 저장하고, 가입 제출(§1-3)이 그 마커를 대조한 뒤 **소비(삭제)** 한다. 마커의 소비처가 §1-3 하나뿐이라 용도 구분 필드를 두지 않는다 — **키스페이스가 곧 용도**다(§1-2·§1-6과 같은 규칙).

> **챌린지 부재(미발송·만료·이미 검증)**: 해당 이메일의 챌린지가 없으면 — 인증번호를 한 번도 요청하지 않았거나, TTL 만료, 이미 검증 완료로 소멸, 발송 실패(`502`)로 미저장 — 올릴 `attempts` 레코드 자체가 없으므로 **즉시 `422 AUTH_EMAIL_VERIFICATION_FAILED`** 로 거절하고 인증번호 (재)요청(§1-11)을 유도한다.
> **불일치·만료·시도 상한(5회) 초과를 모두 `422` 한 코드로 응답**한다 — 시도 초과를 `429`로 분리하는 정식 사용자 트랙(§4)과 다른 점이다. 비로그인 경로라 챌린지의 존재·시도 상태를 응답으로 구분해 주지 않는다(§1-2와 같은 판단).
> **여기서는 중복을 다시 보지 않는다** — 중복 판정은 §1-11이 이미 했고, 이 단계에서 또 갈라 봐야 새로 알려 주는 사실이 없다. 확인은 "받은 번호가 맞는가"만 답한다.
...
> 검증 마커는 30분간 유효하다 — 그 안에 §1-3을 제출해야 하며, 만료되면 가입 제출이 `422 AUTH_EMAIL_NOT_VERIFIED`로 거절되고 §1-11부터 다시 한다.

#### 발생 가능한 에러

| status | code | 시점 |
| --- | --- | --- |
| 400 | `INVALID_INPUT` | `email`/`code` 누락·빈값, `email` 형식 위반 |
| 400 | `MALFORMED_REQUEST` | JSON 파싱 불가/타입 불일치 |
| 422 | `AUTH_EMAIL_VERIFICATION_FAILED` | 코드 불일치, 만료, **검증 시도 상한(5회) 초과**, 또는 챌린지 부재(미발송·만료·이미 검증) — 정식 사용자 트랙(§4)과 달리 시도 초과도 이 코드다 |
