...
> **그 생략은 저절로 되지 않는다.** `getUserName`은 이름이 없을 때만 빈 문자열을 주고, **계정 행이 없거나 탈퇴했으면 `UserNotFoundException`을 던진다**. 그대로 흘리면 심사 목록이 통째로 `404 USER_NOT_FOUND`가 되는데, 그 코드는 "매물이 없다"도 "관리자가 없다"도 아니라 관리자 화면이 해석할 수 없다. 그래서 **`listing`이 그 예외만 잡아 이름을 비운다** — 같은 쿼리를 쓰는 `chat`·`booking`은 잡지 않는데, 거기서는 상대가 없으면 그 대화·예약 자체가 성립하지 않아 실패가 곧 정답이기 때문이다. 심사는 반대다: 임대인 계정이 사라져도 그 매물은 여전히 판단해야 할 대상으로 남는다.
