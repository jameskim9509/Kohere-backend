...
- 선행: 가입용 연락처 인증(US-1-13)과 **가입용 이메일 인증(US-1-18)** 을 **둘 다** 통과해야 제출할 수 있다(서로 순서는 무관하다). 앱 소셜 로그인(US-1-1)·약관 동의(US-1-7)·임대인 온보딩(US-1-9)은 선행이 아니다 — 웹 가입은 앱과 독립된 진입점이며, 이미 앱 계정이 있는 사람도 같은 폼을 그대로 제출한다(연동 여부로 입력란이 갈리지 않는다).
...
- **보안 — 이메일 인증 없는 가입 시도(복구 경로 보장)**
  Given 제출한 `email`에 유효한 가입용 검증 마커(US-1-18)가 없으면
  When `POST /api/v1/auth/signup`을 호출하면
  Then `422` + `error.code=AUTH_EMAIL_NOT_VERIFIED`를 반환하고 **계정 생성도 연동도 하지 않는다**. 연락처 게이트가 *남의 계정*을 지키는 것과 달리 이 게이트가 지키는 것은 **본인의 복구 경로**다 — 닿지 않는 주소로 가입한 계정은 잠기는 순간 비밀번호 재설정(US-1-17)을 받을 수 없어 자력 복구가 불가능해진다. 두 게이트는 서로 순서가 무관하며, 둘 다 없으면 연락처 쪽 코드가 먼저 나간다(기존 계약 유지).
- **비즈니스 규칙 — 필수 약관 미동의**
...
- **비즈니스 규칙 — 이메일 중복(로그인 ID 유일성)**
  ...
  When 가입을 제출하면
  Then `409` + `error.code=AUTH_EMAIL_ALREADY_REGISTERED`를 반환한다. 이메일은 연동 키가 아니라 **웹 로그인 ID**이므로 유일해야 로그인(US-1-12)이 계정을 특정할 수 있다(중복을 허용하면 로그인 기능 자체가 성립하지 않는다). **US-1-18이 발송 단계에서 먼저 걸러도 이 게이트는 남는다** — 인증번호를 받고 제출하기까지 최대 30분 동안 남이 같은 주소로 가입을 마칠 수 있고, 이 게이트를 빼면 그 창에서 나가는 코드가 "다른 이메일을 쓰라"가 아니라 "잠시 후 재시도하라"(공통 충돌 코드)로 바뀌어 **재시도로 절대 풀리지 않는 상황에 재시도를 권하게 된다**.
...
- **개인정보 — 미가입 이메일도 동일한 200이다**
  ...
  Then 가입된 이메일과 **완전히 같은** `200 OK` + `data.expiresIn=1800`을 반환하고 **메일만 보내지 않는다**. 이 경로에는 선행 게이트가 없어 임의의 이메일로 부를 수 있으므로, 응답을 가르는 순간 완전한 계정 열거 오라클이 된다(US-1-16이 `404`를 낼 수 있는 것은 SMS 마커 뒤라서이지 여기와 같은 자리가 아니다). **단 그 오라클은 US-1-18이 별도로 제공하게 됐다** — 가입용 이메일 인증 발송이 같은 자리에서 이미 가입된 주소에 `409`를 준다. 그래서 여기서 계속 감추는 것은 **깊이 방어**이지 열거 차단이 아니다.
...
### US-1-18 — 가입 전 이메일 인증하기 (임대인 웹 전용, 비로그인)

**As a** 아직 로그인하지 않은 웹 가입자
**I want** 가입에 쓸 이메일로 인증번호를 받아 확인하기를
**So that** 그 주소가 실제로 내 메일함임을 증명하고 웹 회원가입을 진행할 수 있다.

- 우선순위: **High**
- 관련 NFR: 보안(이메일 소유 인증·인증번호 해시 보관·**비로그인 경로 메일 남용 방지**), 보안(이메일·인증번호 로그 마스킹)
- 선행: 없다(비로그인 진입점). 이 인증을 통과해야 웹 회원가입(US-1-11)을 제출할 수 있다. 가입 전 연락처 인증(US-1-13)과 **서로 순서가 무관**하며 둘 다 통과하면 된다.
- 왜 필요한가: 웹 이메일은 **로그인 ID이자 비밀번호 재설정 메일이 도착할 유일한 주소**이고, 그 재설정이 계정 잠금 해제를 겸한다(US-1-17). 지금은 아무 검증 없이 가입되므로 오타로 등록한 계정은 **비밀번호를 반복 실패해 잠기는 순간 본인이 풀 방법이 사라진다**(운영자가 DB를 직접 고쳐야 한다). 가입 시점에 한 번 확인시키는 것이 그 막다른 길을 없애는 유일한 지점이다.
- 백엔드 관점: 정식 사용자용 이메일 인증(US-1-6)은 챌린지를 **`userId`를 키로 저장**하고 `ROLE_USER` 뒤에 있어 계정이 없는 가입 전 단계에서 재사용할 수 없다. 그래서 **정규화한 이메일을 키로 쓰는 챌린지**를 쓰는 엔드포인트 `POST /api/v1/auth/email/signup/verification-code`·`POST /api/v1/auth/email/signup/verify`를 신설하고 `permitAll`로 연다. 기존 `/auth/email/verification-code`·`/auth/email/verify`는 그대로 둔다.
  - 인증번호 정책(6자리·코드 TTL 5분·검증 마커 TTL 30분·검증 시도 상한 5회·재발송 간격 60초)과 `VerificationEmailSender` 포트·해시 보관·동기 발송 실패 정책은 US-1-6과 **동일하게 재사용**한다.
  - 이메일은 저장·조회 전에 **앞뒤 공백을 지우고 소문자로 접어 정규화**하므로 표기가 달라도 같은 챌린지를 가리킨다. 로그인 ID 테이블의 collation이 대소문자를 구분하지 않으므로 DB 동작과도 일치한다.
  - **발송 단계에서 로그인 ID 중복을 판정한다** — 이미 가입된 주소면 메일을 보내지 않고 `409`로 끊는다. 감추면 **남의 메일함으로 인증번호가 발송**되고(이메일 채널은 그 발송 자체가 피해다), 사용자는 가입 제출까지 가서야 중복을 알게 된다.
  - 검증 마커의 소비처는 **웹 회원가입 제출(US-1-11) 하나뿐**이라 용도 구분 필드를 두지 않으며, 가입이 성공하면 마커를 즉시 소비(삭제)한다. 연락처 인증(US-1-13)과 **키스페이스를 나눠** 서로의 게이트를 통과시키지 못하게 한다.
  - 앱 심사용 고정 인증번호 우회는 `userId`와 Google 소셜 계정을 전제로 하므로 **이 비로그인 경로에는 적용하지 않는다**(US-1-13과 같은 판단).
  - **인증 절차 자체에는 환경별 토글이 없다** — 모든 실행 환경에서 같은 흐름이 돈다. 설정으로 가르는 것은 **메일을 실제로 보낼지, 인증번호를 로그로 남길지**(발송 채널)뿐이며, 이는 SMS 발송이 이미 쓰고 있는 방식과 같다.

**AC (Given / When / Then)**

- **정상 — 인증번호 발송**
  Given 비로그인 상태에서 아직 가입되지 않은 이메일을 입력하고
  When `POST /api/v1/auth/email/signup/verification-code`에 `{ email }`을 담아 호출하면
  Then `200 OK` + `data.email`(마스킹)·`data.expiresIn`(만료 초)을 반환하고 해당 주소로 인증번호를 발송한다. 발송에 성공한 뒤에만 챌린지를 저장하며, 인증번호 원문은 저장·로그하지 않고 해시로만 보관한다.
- **정상 — 인증번호 확인**
  Given 발송된 인증번호가 유효(미만료·시도 미초과)하고
  When `POST /api/v1/auth/email/signup/verify`에 `{ email, code }`를 담아 호출하면
  Then `200 OK` + `data.email`(마스킹)·`data.verified=true`를 반환하고 **정규화한 이메일을 키로 검증 마커(TTL 30분)를 저장**해 웹 회원가입 제출(US-1-11)에서 대조할 수 있게 한다.
- **비즈니스 규칙 — 이미 가입된 이메일**
  Given 그 주소가 이미 웹 로그인 ID로 쓰이고 있으면
  When `POST /api/v1/auth/email/signup/verification-code`를 호출하면
  Then `409` + `error.code=AUTH_EMAIL_ALREADY_REGISTERED`를 반환하고 **메일을 보내지 않으며 챌린지도 만들지 않는다**. 판정은 웹 자격증명 테이블만 보고 회원 프로필의 이메일은 보지 않는다(앱 소셜과 같은 주소로 웹 가입하는 것이 가장 흔한 정상 경로다).
- **비즈니스 규칙 — 인증번호 불일치/만료/시도 초과**
  Given 잘못된 인증번호이거나 만료(미발송 포함)됐거나 검증 시도 상한(5회)을 넘겼으면
  When `POST /api/v1/auth/email/signup/verify`를 호출하면
  Then `422` + `error.code=AUTH_EMAIL_VERIFICATION_FAILED`를 반환하고 검증 마커를 저장하지 않는다. **시도 초과도 429가 아니라 이 422**다 — 비로그인 경로에서는 코드가 갈리는 것 자체가 챌린지 존재·시도 잔량을 알려 주는 신호다.
- **보안 — 메일 발송 남용 방지(비로그인 경로 필수)**
  Given 재발송 간격 60초를 지키지 않거나, **같은 이메일로 1시간에 5회**를 넘겨 발송하거나, **같은 IP에서 1시간에 20회**를 넘겨 발송하면
  When `POST /api/v1/auth/email/signup/verification-code`를 호출하면
  Then `429` + `error.code=TOO_MANY_REQUESTS`를 반환하고 메일을 보내지 않는다. 어느 축에 걸렸는지 구분해 알리지 않으며(한도 역산 방지), 예산은 가입용 SMS·이메일 찾기·재설정 링크와 **각각 따로** 둔다. 한도 판정은 **중복 검사보다 먼저**여서 열거 관찰 한 번이 예산 한 칸을 쓴다.
- **장애 — 메일 발송 실패**
  Given 메일 provider 장애·타임아웃으로 발송이 실패하면
  When `POST /api/v1/auth/email/signup/verification-code`를 호출하면
  Then `502` + `error.code=UPSTREAM_ERROR`를 반환하고 **챌린지를 저장하지 않는다**(재시도 유도). 챌린지를 먼저 쓰면 메일을 받지 못한 사용자가 만료를 기다려야 하고 재발송 쿨다운만 소모한다.
- **인증·권한 — 토큰이 필요 없다**
  Given 두 엔드포인트 모두 `permitAll` 공개 경로이므로
  When `Authorization` 헤더 없이 호출하면
  Then 정상 처리한다. 보안 설정의 `permitAll` matcher와 **공개 경로 목록 양쪽에** 등록한다(한쪽만 넣으면 만료된 토큰이 남은 브라우저에서만 `401 TOKEN_EXPIRED`가 난다).
- **알려진 제약 — 이메일 가입 여부 열거를 수용한다**
  Given 임의의 이메일로 `POST /api/v1/auth/email/signup/verification-code`를 호출하면
  When 응답이 `409`인지 `200`인지 보면
  Then **그 주소의 가입 여부를 알 수 있다.** 선행 게이트가 없는 경로이므로 이 열거는 막히지 않으며, 레이트리밋은 비용 상한이지 방어가 아니다(이메일 축은 한 주소당 한 번만 물으면 되는 관찰에 무력하고 IP 축은 위조 가능하다). 가입 폼에서 즉시 알리는 값과 **남의 메일함으로 가는 발송을 막는 값**이 더 크다고 보아 수용한다. 그 결과 US-1-17의 재설정 링크 발송이 세워 둔 열거 방어는 이 경로로 우회되며, 그쪽은 깊이 방어로 남는다.
...
- 조회 관점: 목록은 **모든 상태**를 대상으로 하며 `status` 쿼리 파라미터로 상태별 필터가 가능하다(다중 값 허용, 생략하면 전체). 세입자 조회 3종은 저장소에서 `PUBLISHED`를 고정하고 있으므로 재사용하지 않고 **심사 전용 조회 경로**를 따로 둔다 — 세입자 경로의 안전장치를 풀지 않기 위해서다. 기본 정렬은 등록 최신순이고, `sort`로 `createdAt,asc`(등록 오래된 순)와 `updatedAt,desc`(최근 수정순 — 수정 재심사 건이 등록 최신순 큐 바닥에 가라앉는 것을 따로 보는 용도)를 고를 수 있다. **모르는 정렬 키는 `400`이 아니라 조용히 기본값으로** 간다 — 정렬 키 하나 때문에 심사 목록이 멈추지 않게 하려는 것이다.
- 노출 관점: 심사 응답은 **매물 문서에 저장된 모든 필드 + 등록 임대인의 계정 이름**을 담는다 — `landlordId`·`businessRegistrationNumber`·설문 3종·`rejectionReason`까지 세입자 응답이 감추는 값을 감추지 않는다. 표시 여부는 관리자 화면이 정한다. 노출해도 되는 근거는 매물 문서에 **임대인 개인 연락처가 저장되지 않기 때문**이고([ADR-0039](../adr/0039-listing-schema-v4-registration-form.md) Amended), `businessRegistrationNumber`는 오히려 **관리자가 심사에서 진위를 수동 확인해야 하는 값**이다([ADR-0033](../adr/0033-business-registry-verification.md) 개정). 마지막 하나 **`landlordName`만은 매물 문서에서 오는 값이 아니다** — 매물을 등록한 임대인의 계정 이름이다. `landlordId` 숫자만으로는 심사 화면에서 누가 올린 매물인지 알아볼 수 없기 때문이고, 이름도 PII지만 **심사자가 등록자를 식별하는 데 필요한 최소값**이며 연락처는 여전히 싣지 않는다. 이름의 정본은 `users.name`(MySQL)이라 매물 문서(MongoDB)에 **스냅샷으로 저장하지 않고** 조회 시점에 `user :: api`의 기존 공개 쿼리 `getUserName`으로 **애플리케이션 레벨에서 조합**한다(cross-store 조인 금지 — [ADR-0005](../adr/0005-polyglot-persistence.md), 즉시 결과가 필요한 조회는 동기 공개 API — [ADR-0002](../adr/0002-inter-module-communication-via-events.md)). 예약이 예약자 성명을 스냅샷 없이 실시간 조인하는 것과 같은 선택이다(US-4-2) — 이름은 관리자가 **지금** 보는 값이면 되고 이력을 보존할 이유가 없어 스키마를 늘리지 않는다. 목록은 한 페이지 안에서 **임대인별로 한 번만** 조회한다 — 한 임대인이 지점마다 매물을 올리는 것이 정상이라 페이지 안 중복이 흔하다. 이름이 비어 있거나(소셜 provider 미제공·탈퇴 PII 익명화) 계정 행 자체를 찾을 수 없으면 **응답에서 그 키가 빠진다** — 같은 응답의 `serviceFeedback`·`rejectionReason`이 이미 쓰는 규칙이고, 심사 대상은 매물이고 이름은 **표시 보조값**이라 매물 한 건의 임대인 때문에 심사 목록 전체가 `404`로 죽어서는 안 되기 때문이다. `getUserName`은 계정 행이 없으면 예외를 던지므로 그 생략은 **`listing`이 그 예외를 잡아** 이뤄진다(그대로 흘리면 목록 전체가 `404`가 된다). **`listing.contact.managerName`과는 다른 값**이다 — 그쪽은 등록 폼에 임대인이 적어 넣은 **지점 담당자** 이름이고 `landlordName`은 **매물을 등록한 계정**의 이름이라 둘이 같을 수도, 다를 수도 있다. 표시 언어는 임대인 화면과 같이 한국어 고정이다.
...
- 후속(이번 범위 아님): 승인·반려 시 임대인 알림(도메인 이벤트) · 심사 이력 테이블 · 관리자 웹 프론트 · 임대인 기준 검색·필터 — 응답에 이름을 실어도 목록은 여전히 `status`·`page`·`size`·`sort`만 받는다.
...
- 시나리오: 심사 상세는 저장된 전 필드에 임대인 이름까지 준다
  Given 관리자가 심사 대기 매물 하나를 골랐고
  When `GET /api/v1/admin/listings/{listingId}`를 호출하면
  Then `200 OK`로 `landlordId`·`businessRegistrationNumber`·설문 3종·`rejectionReason`을 포함한 전 필드를 반환하고(세입자 상세와 달리 감추지 않는다), 매물 문서에는 없고 조회 시점에 `user :: api`로 붙여 오는 `landlordName`(그 임대인의 `users.name`)이 `landlordId` 바로 다음에 실린다
- 시나리오: 목록 항목마다 등록 임대인의 이름이 함께 온다
  Given 서로 다른 임대인이 올린 매물이 섞여 있고 그중 한 매물의 임대인은 이름이 비어 있거나 계정을 찾을 수 없고
  When `GET /api/v1/admin/listings`를 호출하면
  Then `200 OK`이고 각 항목의 `landlordName`은 그 매물을 등록한 임대인의 `users.name`이며, 이름을 알 수 없는 매물은 **그 항목의 키만 빠지고 나머지 항목은 정상 조회된다** — 이름은 표시 보조값이라 임대인 한 명 때문에 심사 목록 전체가 `404`로 죽지 않는다
- 시나리오: 승인하면 세입자에게 보인다
