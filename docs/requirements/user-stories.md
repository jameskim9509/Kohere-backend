...
- **입력 검증 실패**
  Given `firstName`/`lastName`/`country`가 비었거나(`country`가 `countries`에 없는 ISO 코드이거나), `gender`가 `MALE`/`FEMALE` 외 값이거나, `occupation`이 정의된 enum(`UNDERGRADUATE_STUDENT`·`GRADUATE_STUDENT`·`EXCHANGE_STUDENT`·`EDUCATION_ACADEMIC_RESEARCH`·`IT_SOFTWARE_ENGINEERING`·`DEVELOPER`·`DESIGNER`) 외 값이거나, `birthDate`가 `YYYY-MM-DD` 형식 위반/미래 날짜이거나, `visaType`이 정의된 enum(값=상수명_코드: `DIPLOMATIC_OFFICIAL_A-1_A-2`·`VISA_EXEMPTED_B`·`JOURNALISM_RELIGIOUS_AFFAIRS_C-1_D-5_D-6`·`SHORT_TERM_VISIT_C-2_C-3`·`STUDY_D-2`·`TRAINEE_D-3_D-4`·`INTRA_COMPANY_TRANSFER_D-7`·`PROFESSIONAL_C-4_D-1_D-8_D-9_D-10_E-1_E-2_E-3_E-4_E-5_E-6_E-7`·`NON_PROFESSIONAL_E-8_E-9_E-10`·`WORKING_HOLIDAY_H-1`·`WORK_AND_VISIT_H-2`·`FAMILY_VISITOR_DEPENDENT_F-1_F-2_F-3`·`OVERSEAS_KOREAN_F-4`·`PERMANENT_RESIDENCE_F-5`·`MARRIAGE_MIGRANT_F-6`·`OTHERS_G-1`) 외 값이거나, `email` 형식이 어긋나면
