...
- **경계 — 의도적 계정 잠금(DoS)은 수용한다**
  ...
  When 그런 시도가 반복되면
  Then IP 단위 레이트리밋으로 완화하되 **완전히 막지는 못한다**. 잠금 정책을 채택하는 이상 피할 수 없는 고전적 부작용이며 리스크를 수용하고 진행한다.
- **보안 — refresh 쿠키 속성**
...
- **정상 — 쿠키로 재발급·로그아웃**
  ...
  Then 기존 회전·재사용 탐지 규칙(US-1-3)대로 새 토큰을 발급하고 **회전된 refresh를 다시 쿠키로** 내린다. 본문으로 온 요청은 기존대로 본문에 담아 돌려준다(앱 하위 호환). 로그아웃(US-1-4)은 서버 무효화와 함께 **`Max-Age=0` 삭제 쿠키**를 내린다.
...
- **정상 — 병합 대상이 없는 일반 온보딩**
  ...
  When 임대인 온보딩을 제출하면
  Then **기존 동작 그대로** 자기 계정을 `ACTIVE`로 전이시킨다(US-1-9는 변경되지 않는다).
- **보안 — 연락처 인증 없는 병합 차단**
...
- **정합성 — 병합 원자성**
  ...
  When 온보딩을 제출하면
  Then **전체를 롤백**한다. `social_accounts`가 어느 계정에도 붙지 않은 상태를 남기면 앱 로그인이 영구히 깨진다.
