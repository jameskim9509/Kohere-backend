...
- 상태 관점: 매물 상태는 `PENDING`(심사 대기) · `PUBLISHED`(승인·공개) · `REJECTED`(반려) **3종**이다. **승인은 `PENDING`에서만** 할 수 있고 그 외 상태의 승인 요청은 `409` + `LISTING_INVALID_STATUS_TRANSITION`이다 — 반려된 매물의 재승인은 관리자가 직접 하는 것이 아니라 **임대인이 고쳐 `PENDING`으로 되돌린 뒤** 다시 이 문을 통과한다(수정 API는 US-3-6 후속). 직접 승인을 열면 아무것도 고치지 않은 매물이 통과하는 우회로가 되고 반려 사유가 해소됐는지 확인할 근거가 사라진다. 반면 **반려는 어느 상태에서든** 할 수 있다 — 심사 대기 매물의 1차 반려뿐 아니라 공개 후 문제가 발견된 매물을 내리는 **사후 반려**와 이미 반려한 매물의 **사유 정정**이 모두 정상 경로다. 노출을 **여는** 쪽만 좁게 통제하고 **닫는** 쪽은 열어 두는 것이 안전한 방향이며, 관리자가 서비스에서 매물을 내리는 유일한 수단이기도 하다.
...
- 시나리오: 승인 대상이 아닌 매물은 승인할 수 없다
  Given 매물이 이미 `PUBLISHED`이거나 `REJECTED`이고
  When 관리자가 승인을 호출하면
  Then `409 Conflict`, `error.code=LISTING_INVALID_STATUS_TRANSITION`을 반환한다. 재승인은 임대인이 수정해 `PENDING`으로 되돌린 뒤에만 가능하다
