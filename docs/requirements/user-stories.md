...
- 시나리오: 검색으로 받은 좌표가 그대로 저장된다
  ...
  Then `address.fullAddress`는 보낸 값 그대로이고 `address.city`·`district`는 파싱된 `City`·`District` 코드이며, **`location`은 요청의 좌표(`{ type: "Point", coordinates: [lng, lat] }`)로 채워진다**. **`nearbyUniversityCodes`는 그 좌표 반경 2km 안의 대학 코드로 채워진다** — 예: 신촌 좌표는 `["YONSEI","EWHA","HONGIK"]`, 대학가 밖이면 빈 배열([ADR-0045](../adr/0045-nearby-university-mapping-from-seeded-coordinates.md))
