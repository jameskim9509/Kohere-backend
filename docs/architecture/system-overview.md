...
| MySQL 마이그레이션 | **Flyway**(`flyway-core`,`flyway-mysql`) | 도입 | MySQL 전용. MongoDB는 앱 레벨 버전 필드(ADR-0005 Decision 7). [migration-policy](../database/migration-policy.md)(템플릿) |
| 소프트삭제·PII 보존 | 정책 결정 필요(선행) | 도입 | 스키마·검색 가시성의 전제 → ADR 필요 |
| 데이터 설계 정본 | [database-design](../database/database-design.md)(템플릿) | 추후 | MySQL 스키마 / Mongo 컬렉션 / Redis 키스페이스로 분할 작성 |
...
| 서버 JWT 서명 | jjwt(`io.jsonwebtoken`). **알고리즘 미결**(HS256 유력 vs RS256/ES256) | 도입 | ADR-0003 [L69](../adr/0003-jwt-auth-after-oauth-login.md#L69) → ADR 필요 |
...
| API 문서 | springdoc-openapi | 추후 | 현재는 [api/specs](../api/specs/README.md) Markdown 정본 |
...
- **refresh 토큰 → Redis (완료)**: **[ADR-0006](../adr/0006-refresh-token-store-redis.md)** 작성, ADR-0005 보완·ADR-0003 후속 닫힘.
- **신규/갱신 ADR**: 서버 JWT 서명 알고리즘, 소프트삭제·PII 보존, 추천 랭킹 알고리즘, MySQL 마이그레이션 도구 확정(Flyway 권장), **booking·chat 저장소(F-03)**.
- **brief §7 결정(7/10 사수)**: 임대인 연락 = F-03 신청→인앱 채팅 기록 확정(✅ 본 문서 반영), 커뮤니티 사진 업로드 in/out, Flyway 확정.
- **database-design**: MySQL 스키마 / Mongo 컬렉션·`2dsphere` / Redis 키스페이스로 분할 작성.
...
| 가용성 | RDS·Mongo·Redis 백업/복제, 무중단 배포는 expand-contract 마이그레이션 | [migration-policy](../database/migration-policy.md)(템플릿) |
...
- [code-style](../convention/code-style.md) · [database-design](../database/database-design.md)(템플릿) · [migration-policy](../database/migration-policy.md)(템플릿)
...
> **갱신 현황:** ADR-0006 작성 + ADR-0005/0003 반영 완료(refresh→Redis). **남은 갱신:** database-design(스토어별 스키마) · build.gradle(`data-jpa`+드라이버·`data-mongodb`·`data-redis` 추가) · 시퀀스 02·03(Mongo 반영).

## 체크리스트

- [ ] 다이어그램이 ADR-0005 저장소 배치(Mongo: listing+찜+최근·diagnosis / MySQL: auth·user / Redis: refresh / booking·chat 추후 결정)와 일치한다
