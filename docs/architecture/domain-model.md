...
| --- | --- | --- | --- |
| [`auth`](#1-auth--인증온보딩) | `SocialAccount`, `RefreshToken`, `EmailVerification`, `PhoneVerification`, `BusinessVerification` | `SocialIdentity`, `TokenHash` | ✅ |
...
소셜 로그인(Apple/Google) 자격을 회원 식별자로 매핑하고, 서버 자체 세션 토큰(불투명 refresh)의 발급·회전·재사용 탐지·무효화와 **세입자 온보딩 중 이메일 인증(인증번호 발송·검증)**, 그리고 **임대인 온보딩 중 연락처(휴대폰) SMS 인증·사업자등록번호 검증**을 책임지는 인증 경계다. 회원 프로필·상태(`PENDING`/`TERMS_AGREED`/`ACTIVE`/`WITHDRAWN`)는 `user` 모듈 소관이므로 여기선 회원 식별자(`userId`)로만 참조한다. 온보딩은 역할(`userType`: 세입자 `TENANT` / 임대인 `LANDLORD`)에 따라 분기하며, **소셜 로그인·약관 동의까지는 두 역할 공통**이고 이후 본인 확인이 갈린다 — **세입자는 이메일 인증(`EmailVerification`), 임대인은 연락처 인증(`PhoneVerification`) + 사업자번호 검증(`BusinessVerification`)**. 온보딩 제출 엔드포인트(세입자 `POST /auth/onboarding`, 임대인 `POST /auth/landlord/onboarding`)에서 `userType`이 확정된다.
...
**`BusinessVerification`** — 임대인 온보딩 중 입력한 **사업자등록번호**의 진위·상태(정상 영업)를 외부 사업자등록정보 검증 API(국세청 사업자등록정보 기반, 구체 provider는 [ADR-0033](../adr/0033-business-registry-verification.md))로 확인하는 단명(ephemeral) 검증 시도 애그리거트 루트. 연락처 인증(`PhoneVerification`)·세입자 이메일 인증(`EmailVerification`)과 같은 "별도 검증 → 마커 → 온보딩 대조" 패턴이며 임대인(`userType=LANDLORD` 분기) 전용이다. 비즈니스 키 `userId`(온보딩 중 회원 단위로 1건). 영속 무관이나 단명 상태라 Redis로 물리화한다([database-design](../database/database-design.md) §4-1).

**속성:**

| 속성 | 타입 | 설명 |
| --- | --- | --- |
| `userId` | 식별자 | 검증 대상(임대인 온보딩 중) 회원 → `User` 식별자 참조(시도 단위) |
| `businessNumberHash` | String | 검증 통과한 사업자등록번호의 단방향 해시(SHA-256). 원문 미보관(확인 필요: 해시 알고리즘·솔트) |
| `status` | enum `BusinessVerificationStatus` | 검증 시도 상태 |
| `expiresAt` | Instant | 검증 마커 만료 시각(UTC) |
| `verifiedAt` | Instant, nullable | 검증 통과(`VERIFIED`) 시각(UTC) |

**불변식:** 검증은 온보딩 토큰을 가진 본인에 한함(임대인 분기); 사업자등록정보 검증 API **동기 호출**로 국세청 사업자등록정보 진위·상태를 확인해 **정상(계속 사업자)만** `VERIFIED`로 확정하고, 미등록·휴업·폐업·진위 불일치는 마커를 만들지 않고 `422 AUTH_BUSINESS_NUMBER_VERIFICATION_FAILED`로 거부(외부 장애·타임아웃은 마커 미생성 + `502 UPSTREAM_ERROR`로 재시도 유도 — 이메일 발송 실패와 대칭); 사용자당 활성 시도 1건(재검증은 기존 시도 대체); 임대인 온보딩 제출(`POST /auth/landlord/onboarding`)은 제출 사업자번호가 `VERIFIED`된 번호(`businessNumberHash` 대조)와 일치해야 진행하며 미검증·불일치는 `422 AUTH_BUSINESS_NUMBER_NOT_VERIFIED`; 사업자번호 원문은 보관·로그하지 않고(해시만) 응답·로그는 마스킹(예 `****567890`, 확인 필요); 만료/완료 시도는 TTL로 자동 소멸. (확인 필요: 검증 서비스 회신 상호·대표자 저장 여부, 검증 레이트리밋 임계값, 사업자번호 유니크 제약은 앱 레벨로만 — DB 제약 미적용)
...
| `BusinessVerificationStatus` | `VERIFIED` | 사업자등록정보 검증 API로 사업자등록번호 진위·정상 영업 확인 완료(임대인 온보딩 제출에 사용 가능) — 검증 실패는 마커를 만들지 않으므로 별도 실패 상태 없음 |

**협력 / 이벤트:** 회원 프로필·상태는 `user`가 소유하고 `auth`는 `userId`로만 참조한다(ADR-0002). 신규 분기의 `PENDING` 회원 생성, 약관 동의 시 `TERMS_AGREED` 전이, 온보딩 시 `ACTIVE` 전이, 탈퇴 시 `WITHDRAWN` 전이는 `user`의 공개 명령/쿼리로 협력하고 결과 식별자를 `SocialAccount.userId`로 보유한다. 세입자 온보딩 입력의 `gender`·`visaType`·`occupation`은 `user` 소유 enum이라 타입을 공유하지 않고 원시 값으로 전달한다. `userType`(세입자/임대인 역할)은 온보딩 제출 엔드포인트로 분기·확정돼 `user`로 전달된다(이후 불변). 온보딩 완료 명령 전에 `auth`가 **역할별 검증 게이트**를 우선순위대로 통과시킨다 — **세입자는 약관 동의 → 이메일 인증**(`EmailVerification`의 `VERIFIED` 여부만, 미인증·불일치 `422 AUTH_EMAIL_NOT_VERIFIED`), **임대인은 약관 동의 → 연락처 인증 → 사업자번호 검증**(`PhoneVerification`의 `VERIFIED`(미인증·불일치 `422 AUTH_PHONE_NOT_VERIFIED`)에 이어 `BusinessVerification`의 `VERIFIED`(미검증·불일치 `422 AUTH_BUSINESS_NUMBER_NOT_VERIFIED`))을 확인한다(검증 상태는 `auth` 소유, 확정된 값 — 세입자 `email`, 임대인 `phoneNumber`·`businessRegistrationNumber` — 은 `user`가 보유). 인증번호 메일 발송은 아웃바운드 포트 `VerificationEmailSender`(application)로 추상화하고 인프라 어댑터(SMTP)가 구현한다 — **동기 발송**이라 발송 성공 시에만 챌린지를 확정하고, 발송 실패(provider 장애·타임아웃)는 `502 UPSTREAM_ERROR`로 응답한다(메일 템플릿·다국어는 확인 필요). 연락처 SMS 발송은 아웃바운드 포트 `VerificationSmsSender`(application)로 추상화하고 인프라 어댑터(SMS API — 구체 provider는 [ADR-0034](../adr/0034-landlord-phone-sms-verification.md))가 구현한다 — 이메일과 동일하게 **동기 발송**(발송 성공 시에만 챌린지 확정, 실패 시 `502 UPSTREAM_ERROR`)이다. 사업자등록번호 검증은 아웃바운드 포트 `BusinessRegistryVerifier`(application)로 추상화하고 인프라 어댑터(사업자등록정보 검증 API — 국세청 사업자등록정보 진위·상태 기반, 구체 provider는 [ADR-0033](../adr/0033-business-registry-verification.md))가 구현한다 — **동기 검증**이라 정상(계속) 사업자만 `VERIFIED`로 확정하고, 미등록·휴폐업·진위 불일치는 `422 AUTH_BUSINESS_NUMBER_VERIFICATION_FAILED`, 외부 장애·타임아웃은 `502 UPSTREAM_ERROR`로 응답한다. `user`의 탈퇴 이벤트(`UserWithdrawnEvent`)를 구독해 해당 `userId`의 refresh 토큰을 일괄 무효화한다.
...
| `businessRegistrationNumber` | String | 임대인 사업자등록번호(임대인 온보딩 필수, 세입자 미수집). 온보딩 전 `auth` 사업자등록정보 검증(`VERIFIED`)을 거친 값 — 원문 비저장, SHA-256 해시로만 영속(컬럼 `business_registration_number_hash`)하고 응답·로그 마스킹(예 `****567890`)(확인 필요) |
...
| | `LANDLORD` | 임대인(매물 등록자) — 온보딩에서 `name`(단일)·`phoneNumber`·`businessRegistrationNumber` 수집 |
