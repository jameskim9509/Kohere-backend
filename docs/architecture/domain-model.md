...
| | `LANDLORD` | 임대인(매물 등록자) — 온보딩에서 `phoneNumber`·`birthDate` 수집(단일 `name`·`email`은 세입자와 동일하게 소셜 로그인 시 확정·보유; 사업자등록번호는 추후 매물 등록 시점; `country`=`KR`·`lang`=`ko`는 서버 고정) |
