...
| `rejectionReason` | String, nullable | 반려 사유(관리자 입력·한국어 단일 문자열). `status=REJECTED`일 때만 값이 있고 재제출 시 비운다 — 임대인만 읽는 값이라 번역 대상이 아니다. **임대인이 직접 바꿀 수 없다** — 수정 요청에 칸을 두지 않고, 수정이 성공하면 직전 상태와 무관하게 서버가 무조건 비운다 |
