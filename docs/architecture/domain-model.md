...
| `email` | String | 제공자가 제공한 이메일(검증된 토큰의 `email` 클레임에서 추출) — 소셜 로그인 요청의 `email`을 대조·확정하는 진본이자 심사계정 매칭 등에 쓴다. 민감정보 |
...
| `email` | String | 연락 이메일 — **소셜 로그인 시점에 제공자 토큰의 `email` 클레임으로 확정**(요청 `email`이 토큰 값과 일치해야 하며 provider 진본으로 채운다). 민감정보. 세입자 대상(임대인 email 처리·`NULL` 여부는 #192 범위 밖 — 종전 미수집, [ADR-0034](../adr/0034-landlord-phone-sms-verification.md)). 사용자의 email 수정은 #192 범위 밖(후속 이슈) — 당분간 소셜 로그인 값으로 고정 |
