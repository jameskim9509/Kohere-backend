...
sequenceDiagram
    ...
    alt 토큰 없음/위조
        ...
        TGT->>DB: 대상 존재 조회<br/>(targetType, targetId)
        DB-->>TGT: 대상 레코드·채팅방 참여 여부
        ...
        alt MESSAGE 신고이고 채팅방 미참여
            ...
            REPORT->>DB: 신고 저장<br/>(reporterId,targetType,targetId) 유니크,<br/>status=RECEIVED
            DB-->>REPORT: reportId=9001, createdAt(UTC ISO-8601)
...
- `report 모듈`은 레이트리밋→입력 검증→대상 존재→참여 권한·자기 신고→중복 순으로 평가하며, 대상 존재·참여 권한 검증은 대상을 보유한 `community/chat 모듈`이 `저장소`에서 대상 레코드를 조회해 확인하고, `MESSAGE` 신고는 채팅방 참여자가 아니면 `403 FORBIDDEN`을 반환한다.
- 정상 접수 시 `report 모듈`이 `저장소`에 `(reporterId, targetType, targetId)` 유니크 제약으로 1건만 저장하고 `status=RECEIVED`로 `201 Created`를 응답한다(응답에 `reporterId`/`detail` 미노출).
