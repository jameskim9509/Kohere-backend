...
- **`listing` 모듈 코드 변경은 0건이다** — ② 지역 게이트와 ④ 추천 조회가 부르는 `recommendByCriteria`는 애초에 userId 인자가 없고 `RecommendationCriteria`에도 신원 컴포넌트가 없으며, 추천 요청당 `favorite`·`recentListing` 쓰기도 0건이라 게스트 트래픽이 정크를 남기지 않는다.
