...
sequenceDiagram
    ...
    DB-->>DIAG: 저장된 단계별 답
    Note over DIAG: 저장된 답 재검증<br/>(region 1택, purpose 1택,<br/>STUDY면 university 필수·NON_STUDY면 district 필수<br/>(목적에 맞는 하나만, 위반=INVALID_INPUT),<br/>conditions 최대 3개,<br/>monthlyRentMin·monthlyRentMax 각 0 이상 정수 AND monthlyRentMin ≤ monthlyRentMax)
    alt 검증 통과
