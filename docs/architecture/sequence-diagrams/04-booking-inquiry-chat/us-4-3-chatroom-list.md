...
- 세입자가 `GET /api/v1/chat-rooms`(선택 `category`·`page`·`size`)를 호출하면 **chat 모듈**이 **저장소**에서 요청자가 참여한 방만 `lastMessageAt` 내림차순으로 오프셋 페이지네이션해 조회·반환한다.
- 각 항목에 `listing.thumbnailUrl`·`counterpart`·`lastMessage`·`unreadCount`가 담기고 `page` 메타(`number`/`size`/`totalElements`/`totalPages`/`hasNext`)를 포함해 chat 모듈이 `200 OK`로 응답한다. 매물 썸네일 등은 chat 모듈이 저장소에 보유한 비정규화 뷰로 응답하므로 listing 모듈 조회가 필요 없다.
