...
sequenceDiagram
    ...
    alt 승인인데 PENDING이 아님
        LIST-->>W: 409 LISTING_INVALID_STATUS_TRANSITION
        Note over LIST: 재승인은 임대인이 수정해 PENDING으로<br/>되돌린 뒤에만 가능하다(후속 이슈)<br/>반려는 이 분기에 걸리지 않는다
    else 승인이고 PENDING이거나, 반려(상태 무관)
...
- **반려만 상태를 가리지 않는 이유.** 노출을 **여는** 쪽(승인)은 좁게 통제해야 한다 — 아무것도 고치지 않은 매물이 통과하는 우회로를 막기 위해서다. 반대로 **닫는** 쪽(반려)은 언제든 열어 두는 편이 안전하다. 문제가 발견된 공개 매물을 즉시 내릴 수 있어야 하고, 그것이 관리자가 서비스에서 매물을 내리는 유일한 수단이다.
