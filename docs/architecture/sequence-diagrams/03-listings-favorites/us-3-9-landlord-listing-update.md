                ...
                alt 심사 대기 중 (PENDING · UPDATE_PENDING)
                    ...
                    alt 동의 2종이 모두 true가 아님
                        ...
                        alt 원본 없음 (오타 · 7일 만료) · 저장소 장애
                            ...
                            Note over LIST: rejectionReason 소거는 도메인 메서드 안에서 한다<br/>서비스가 지우게 하면 다음 수정 경로가 빠뜨린다
...
- **`rejectionReason`을 무조건 지우는 이유.** 원래 상태를 보지 않고 항상 `null`로 만든다. 분기가 없으면 다음 수정 경로가 빠뜨릴 자리도 없고, 관리자 승인이 이미 같은 관용구를 쓴다. 임대인이 이 값을 요청으로 보낼 칸은 애초에 없다.
