...
- 지도 패닝 시 `GET /api/v1/listings/map`을 호출해 `listing` 모듈이 저장소에서 bbox(또는 center+radius) 영역의 매물을 조회·줌 레벨 기준 클러스터로 집계한다(`200 OK` + `data.clusters[]`/`total`).
- bbox 4좌표 불완전·모순(`swLat>neLat`) 또는 bbox·반경 모드 혼용은 `listing` 모듈이 `400 LISTING_INVALID_BBOX`로 거부한다(검증 실패 분기는 저장소 접근 없음).
