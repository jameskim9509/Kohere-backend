...
- **좌표는 검색 결과를 되돌려 받아 저장한다** — 요청의 `address.lat`·`lng`가 `location`이 된다. 서버는 등록 시점에 지오코딩을 다시 하지 않는다(등록마다 외부 왕복과 502 경로가 생기는 것을 피한다 — [ADR-0042](../../../adr/0042-road-address-search-with-ncp-geocoding.md) §2). 좌표 위조는 관리자 승인 심사가 흡수한다. 좌표는 저장 계약의 **필수** 필드라 좌표 없는 매물은 아예 저장되지 않는다(changeUnit `0116`). `nearbyUniversityCodes`는 아직 빈 배열이다(좌표 기반 파생은 후속).
