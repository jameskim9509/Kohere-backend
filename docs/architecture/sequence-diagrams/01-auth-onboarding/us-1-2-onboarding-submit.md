...
sequenceDiagram
    ...
    alt 필수 약관 동의 누락
        ...
        AUTH->>DB: 사용자 PENDING→ACTIVE 갱신 + refreshToken 해시 저장
        DB-->>AUTH: 갱신 완료
...
- 정상이면 저장소에서 **사용자 상태를 PENDING→ACTIVE로 갱신하고 refreshToken 해시를 저장**한 뒤 `200 OK`로 완성 프로필과 정식 access/refresh 토큰을 발급한다.
