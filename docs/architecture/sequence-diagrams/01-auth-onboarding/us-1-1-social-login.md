...
sequenceDiagram
    ...
    alt 검증 실패 (서명·iss·aud·exp 중 하나)
        ...
        S->>DB: providerUserId로 회원 조회
        DB-->>S: 기존 ACTIVE 사용자
        Note over S: accessToken+refreshToken 발급
        S->>DB: refreshToken 해시 저장
        DB-->>S: 저장 완료
...
- 검증을 통과하면 저장소에서 `providerUserId`로 회원을 조회한다. 기존 ACTIVE 회원이면 발급한 **refreshToken 해시를 저장**하고 `200 OK`로 access+refresh 토큰과 `onboardingRequired=false`를, 신규면 저장소에 **PENDING 사용자를 저장**하고 온보딩 전용 access 토큰과 `onboardingRequired=true`(`refreshToken=null`)를 반환한다. 앱은 그 값으로 홈/온보딩 화면을 분기한다.
