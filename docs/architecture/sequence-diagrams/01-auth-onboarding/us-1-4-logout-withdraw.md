...
sequenceDiagram
    ...
    alt 로그아웃
        ...
        AUTH->>DB: 사용자 WITHDRAWN 갱신 + 토큰 일괄 무효화
        DB-->>AUTH: 갱신 완료
        AUTH-->>C: 204 No Content
        C-->>U: 계정 정리 완료
    end
...
- 로그아웃은 access 토큰으로 `auth 모듈`의 `POST /api/v1/auth/logout`에 `refreshToken`을 담아 호출하면 저장소에서 해당 **refreshToken을 무효화**하고 `204 No Content`를 반환한다(이미 무효화면 멱등).
- 회원 탈퇴는 `auth 모듈`의 `DELETE /api/v1/users/me` 호출 시 저장소에서 **사용자 상태를 WITHDRAWN으로 갱신하고 모든 refresh 토큰을 일괄 무효화**한 뒤 `204 No Content`를 반환한다.
