# US-8-2 · US-8-3 — 주제별 생활 팁(제목·내용·사진) 조회 + 국가 기반 번역
...
- 번역 언어 결정을 위한 표시 언어는 `user`가 보유하며, `lifetip` 모듈은 JWT 클레임에 의존하지 않고 **항상 `user`의 공개 query(`getLanguage(userId)`)를 동기 호출**해 표시 언어(`lang`)를 취득한다(`user`가 `countries.lang`으로 도출, ADR-0002 Decision 5 · 모듈 의존 `lifetip → user` 추가 · `Accept-Language`·토큰 클레임 미사용).
