...
sequenceDiagram
    ...
    alt 인증 누락/만료/위조
        ...
        DB-->>TIP: 주제 카탈로그(각 _id=code, name 언어-키 맵, order)
        Note over TIP: 주제 수는 고정·소규모 → 페이지네이션 없이 전체 배열<br/>(api-design-guide §4 목록 규약 미적용, US-7-3과 동일 성격)
        loop 각 주제(노출 순서 order)
            alt 도큐먼트에 그 언어 키 존재
                Note over TIP: name 언어-키 맵에서 그 언어 값 선택<br/>name=name[lang] (code는 언어 무관·불변)
            else 미지원 언어
                Note over TIP: name을 영어(en)로 폴백<br/>(에러 아님 — 기본 언어=영어)
            end
        end
        TIP-->>C: 200 OK<br/>data.topics[]: { code(UPPER_SNAKE),<br/>name(번역된 표시명) }<br/>(노출 순서대로, 페이지 객체 없음)
        C-->>U: 주제 목록 표시(내 언어)
    end
```
