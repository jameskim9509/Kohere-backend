...
sequenceDiagram
    ...
    COMM->>DB: 게시글·작성자 조회(postId)
    DB-->>COMM: 게시글, 작성자 userId
    COMM->>CHAT: 1:1 채팅방 생성/조회(상대 userId)
    Note over CHAT: NEIGHBOR 카테고리 1:1 방 보장(04 스펙 재사용)
    CHAT->>DB: 1:1 채팅방 생성 저장(없으면 생성, 있으면 조회)
    DB-->>CHAT: chatRoomId, created(신규 여부)
...
- POST /api/v1/community/posts/{postId}/chat으로 community 모듈이 저장소에서 게시글·작성자를 조회한 뒤, chat 모듈에 NEIGHBOR 카테고리 1:1 방 생성/조회를 동기 호출하고, chat 모듈이 저장소에 1:1 채팅방을 생성 저장(없으면 생성, 있으면 조회)해 chatRoomId와 신규 여부를 돌려준다. 신규면 201(created=true), 기존 방이면 200(created=false)으로 멱등하게 반환한다.
