...
- **영속은 폴리글랏**: 문서·지오성(매물 `listing`+`favorite`+`recent-listing`, 진단 `diagnosis`)은 **MongoDB**, 관계·트랜잭션성(`auth`·`user`·`community`)은 **MySQL**. cross-store 조인·트랜잭션 금지(애플리케이션 레벨 조인). 결정: [ADR-0005](docs/adr/0005-polyglot-persistence.md).
