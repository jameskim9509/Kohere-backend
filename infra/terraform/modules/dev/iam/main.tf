# dev 호스트 IAM — SSM(관리자 접속·파라미터 읽기) + ECR pull + S3(이미지·프론트 릴리스) + 로그 반출.
...
# 시크릿 파라미터(이 접두사)만 읽기 + SecureString 복호화(aws/ssm 키) + S3(이미지·프론트 릴리스) + 로그 반출.
data "aws_iam_policy_document" "params" {
  ...
  # 콘텐츠 이미지 버킷 읽기/쓰기(앱이 인스턴스 역할로 업로드).
  statement {
    sid       = "ImagesBucketRW"
