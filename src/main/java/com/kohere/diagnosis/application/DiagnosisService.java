 ...
 * 전이한다({@link #submit}). 답 적용·조건 매핑·문항 번역·③ 분기는 공유 컴포넌트({@link DiagnosisAnswerApplier}·{@link
 * DiagnosisCriteriaMapper}·{@link DiagnosisQuestionTranslator})에 위임한다 — v2 서버 주도 흐름({@link
 * DiagnosisFlowService})과 같은 규칙을 공유한다. 추천은 listing 공개 쿼리({@link ListingRecommendationService})를 동기
 * 호출한다(ADR-0002 D5).
 *
...
public class DiagnosisService {
  ...
  public QuestionResponse getQuestion(long userId, int step) {
    ...
    return questionTranslator.translate(question, resolveLanguage(userId));
  }
  ...
  public RecommendationResponse getRecommendations(
    ...
    RecommendationCriteria criteria = criteriaMapper.toCriteria(diagnosis, page, size, sort);
