 ...
 *
 * <p><b>진단 문서를 전량 지운다.</b> 남은 문서에 {@code conditions: ["NO_ARC"]}가 섞여 있는데(운영 실측 40건) 도메인 {@code
 * DiagnosisCondition}에는 그 상수가 없다 — {@code NO_ARC}를 추가하며 백필했던 커밋이 나중에 상수만 되돌리고 데이터는 두고 갔다. 그 문서를 읽으면
 * enum 변환이 실패하므로, 이력·최근·상세 조회가 그 사용자에게서 통째로 깨진다.
 *
 * <p>값만 {@code $pull}하지 않고 문서를 지우는 이유는 <b>이 컬렉션이 아직 버려도 되는 시험 데이터</b>이기 때문이다. 지우고 나면 같은 문제가 다시 생길 수
 * 없다 — {@code NO_ARC}를 {@code conditions}에 넣을 수 있는 코드 경로가 0이라 새로 쌓이는 문서에는 그 값이 들어가지 않는다. 그래서 읽기 쪽에
 * 미등록 값을 걸러 내는 방어를 따로 두지 않는다.
 *
