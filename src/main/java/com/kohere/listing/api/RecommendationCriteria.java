 ...
 * district}로 좁힌다.
 *
 * <p>대학 조건은 두 방향으로 온다 — 특정 그룹을 고르면 그 멤버 코드가 {@code universityCodes}에 담겨 <b>포함</b> 매칭이 되고, "그 외
 * 대학"({@code ETC})을 고르면 목록에 든 코드 전부가 {@code excludedUniversityCodes}에 담겨 <b>제외</b> 매칭이 된다. 둘 다 비어
 * 있으면 대학 필터가 없다(비유학 등). 진단 지역({@code district})의 {@code ETC}가 명시 5구의 여집합인 것과 같은 규칙이다.
 *
 ...
 * @param universityCodes 진단 ③ 대학 그룹에서 펼친 개별 대학 코드 집합(예: {@code "SNU"}, {@code "CAU"}) — 이 중 하나라도
 *     인근이면 매칭
...
public record RecommendationCriteria(
  ...
  public RecommendationCriteria {
    ...
    if (!universityCodes.isEmpty() && !excludedUniversityCodes.isEmpty()) {
