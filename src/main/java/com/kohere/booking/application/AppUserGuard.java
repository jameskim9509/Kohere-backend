...
import com.kohere.booking.domain.AppUserOnlyBookingException;
...
public class AppUserGuard {
   ...
   *
   * @param userId 요청 주체
   * @throws AppUserOnlyBookingException 세입자·임대인이 아닌 경우(관리자 등)
   */
  public void requireAppUser(long userId) {
    ...
    if (!USER_TYPE_TENANT.equals(userType) && !USER_TYPE_LANDLORD.equals(userType)) {
      throw new AppUserOnlyBookingException();
    }
  }
}
