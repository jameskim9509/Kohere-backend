 ...
 *
 * <p>세입자·임대인 공용 뷰다 — {@code name}은 세입자·임대인 공통 단일 이름이다(#192). {@code userType}에 따라 갈리는 건 세입자 전용
 * 필드(성별·국적·직업·비자정보)와 임대인 전용 마스킹된 {@code phoneNumber}뿐이다. {@code null} 필드는 응답에서 생략한다({@link
 * JsonInclude.Include#NON_NULL}) — 세입자 응답엔 {@code phoneNumber}가, 임대인 응답엔 세입자 전용 필드가 나타나지 않는다.
 *
